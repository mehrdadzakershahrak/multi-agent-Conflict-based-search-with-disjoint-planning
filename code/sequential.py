        ...
        for i in range(self.num_of_agents):  # Find path for each agent
            ...
            for sid, place in enumerate(path):
                for agent in range(i, self.num_of_agents):
                    constraints.append({'agent' : agent + 1, 'loc': [place], 'timestep': sid, 'positive': False})
                    #constraints.append({'agent' : agent + 1, 'loc': [place], 'timestep': sid+1, 'positive': False})
                    if sid < len(path) - 1:
                        constraints.append({'agent' : agent + 1, 'loc': [path[sid], path[sid+1]], 'timestep': sid+1, 'positive': False})
                        constraints.append({'agent' : agent + 1, 'loc': [path[sid+1], path[sid]], 'timestep': sid+1, 'positive': False})
